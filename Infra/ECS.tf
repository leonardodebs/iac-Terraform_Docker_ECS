...
resource "aws_ecs_service" "Django-API" {
  ...
  depends_on = [
    aws_lb.alb,                    # Garante que o Load Balancer seja criado primeiro
    aws_lb_target_group.alvo,      # Garante que o Target Group seja criado primeiro
    aws_lb_listener.http          # Garante que o Listener esteja criado antes
  ]
}
